...
const ProgressIndicator = struct {
    ...
    fn startProgressDialog(self: *ProgressIndicator, metadata: AppMetadata) !void {
        // On Windows, avoid a console spinner thread. The extractor emits a lot of
        // debug output on the main thread during install, and concurrent
        // std.debug.print calls can deadlock the runtime.
        if (builtin.os.tag == .windows) {
...
fn extractAndInstall(allocator: std.mem.Allocator, compressed_data: []const u8, metadata: AppMetadata, self_extraction_dir: []const u8, app_dir: []const u8) !bool {
    ...
    var progress: ?ProgressIndicator = null;
    if (builtin.os.tag != .windows) {
        progress = ProgressIndicator.init(allocator, metadata);
    }
    defer if (progress) |*p| p.deinit();

    const exe_path = if (builtin.os.tag == .macos)
        try std.fs.selfExePathAlloc(allocator)
    else
        "";
    defer if (builtin.os.tag == .macos) allocator.free(exe_path);
