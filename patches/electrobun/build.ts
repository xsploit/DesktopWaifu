...
async function buildSelfExtractor() {
	...
	if (CHANNEL === "debug") {
		...
		// The Windows self-extractor has been unstable under optimized builds,
		// deadlocking before extraction completes. Use a debug extractor there
		// until the optimizer-specific runtime bug is fixed.
		const optimizeMode =
			OS === "win" ? "-Doptimize=Debug" : "-Doptimize=ReleaseSmall";
		await $`cd src/extractor && ../../vendors/zig/zig build ${optimizeMode} ${zigArgs}`;
	}
}
